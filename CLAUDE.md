...
```
golang-dev-skills/
├── golang-dev-plugin/          # 英文版插件（skills 实体存放于此）
│   ├── .claude-plugin/
...
│       ├── golang-guideline/
│       └── mockey-ut/
├── golang-dev-plugin-cn/       # 中文版插件（skills 实体存放于此）
│   ├── .claude-plugin/
│   │   └── plugin.json
│   ├── .codex-plugin/
│   │   └── plugin.json
...
│   └── marketplace.json
├── .agents/plugins/            # OpenAI Codex 市场清单
│   └── marketplace.json
├── .openclaw-plugin/           # OpenClaw 市场清单
│   └── marketplace.json
...
> CC Switch 和 Hermes 可通过 GitHub 仓库扫描发现 skill（路径需指向 `golang-dev-plugin/skills/` 或 `golang-dev-plugin-cn/skills/`）。
...
- Plugin 目录（`golang-dev-plugin/` 和 `golang-dev-plugin-cn/`）名称也遵循小写字母、数字和连字符规则
- `plugin.json` 中的 `name` 字段必须与插件目录名一致
...
4. **更新 marketplace 清单** — 新增、删除、重命名 skill 或 plugin 时，**必须同步更新**以下所有 marketplace 清单文件：
   ...
   | 文件 | 所属工具 | 格式要点 |
   | :--- | :--- | :--- |
   | `.claude-plugin/marketplace.json` | Claude Code | `plugins` 数组，每项含 `name`、`source`（指向插件目录）、`category` |
   | `.agents/plugins/marketplace.json` | OpenAI Codex | `plugins` 数组，每项含 `source`（含 `source`/`path` 指向插件目录）、`policy`、`category` |
   | `.openclaw-plugin/marketplace.json` | OpenClaw | `skills` 数组，每项为指向插件内 `skills/` 的相对路径 |
...
| 工具 | 适配文件 | 所属 |
| :--- | :--- | :--- |
| Claude Code | `<plugin>/.claude-plugin/plugin.json` | 每个插件必须包含 |
| Codex CLI | `<plugin>/.codex-plugin/plugin.json` | 每个插件必须包含 |
